...
| `/labs/header-grader` | Paste the response headers you actually got and get a weighted grade across eight of them, with a reason for every mark lost. It grades a paste rather than fetching a URL, so it works on staging and on internal hosts. |
| `/labs/oauth-flow` | Walks the authorization code flow with real PKCE values generated by WebCrypto, then runs five attacks against it — stolen code, login CSRF, loose redirect_uri, code replay and provider mix-up — showing which check refuses each. Also audits an authorize URL you paste. |
| `/labs/ssh-keys` | Both fingerprints (SHA256 and MD5), the real modulus size, authorized_keys options, and the key type recorded inside the blob checked against the label on the line. Refuses a private key and says what to do about it. |
| `/labs/saml-response` | Decodes a SAMLResponse — base64, URL-encoded or DEFLATE-compressed — and reports the envelope, subject, validity window and attributes, plus which elements are actually signed, XML signature wrapping and the NameID comment trick. The signature is not verified and it says so. |
| `/labs/kerberos-flow` | Walks AS-REQ through AP-REP, then runs Kerberoasting, AS-REP roasting, Golden and Silver tickets, pass-the-ticket and unconstrained delegation — each with what actually fixes it. RC4 against AES256 side by side is the argument for disabling RC4. |
| `/labs/threat-hunt` | A fresh domain of authentication events every run, with one human account compromised before the others. The log starts hidden and every question you ask is counted. No score — a debrief naming the shortest honest path. |
