/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   The LABS form lives in exactly one place (the Labs hub). Every other lab
   page links to it with ?from=<slug>, so a report still says which playground
   it came from without nine near-identical copies of the same form diluting
   the unique content on each page.

   The Games hub carries its own separate form (#games-feedback, generated by
   scripts/games.js). This file governs the Labs one ONLY and must not be
   loaded on /games: it hardcodes lab-feedback-form, lab-feedback-context and
   the '#lab-feedback' hash, so on a page carrying the games form under those
   ids it would rewrite the template with a "Lab:" line and mislabel every
   report. Game PAGES still link here with ?from=<slug>&area=games, which is
   what the area branch below is for.
