...
class API {
  ...
  async compile(options) {
    ...
    // PATCHED (krunalkumar.dpdns.org): the upstream demo always compiles as
    // C++. /labs/c must compile as C, so the language is a caller option.
    return await this.run(clang, 'clang', '-cc1', '-emit-obj',
                          ...this.clangCommonArgs, '-O2', '-o', obj, '-x',
                          options.lang || 'c++', input);
  }
  ...
  async compileLinkRun(contents, lang) {
    // PATCHED: language is threaded through so /labs/c and /labs/cpp share
    // one toolchain without pretending C is C++.
    const input = (lang === 'c') ? `main.c` : `main.cpp`;
    const obj = `main.o`;
