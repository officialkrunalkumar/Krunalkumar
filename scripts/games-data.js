...
const GAMES = [
  ...
  },
  {
    ...
    facts: [
      ...
      'Arm counts computed, not hardcoded',
      'Save the figure as a PNG',
    ],
    hud: [
      { key: 'turn', label: 'Turn', accent: true, init: '137.508°' },
      { key: 'arms', label: 'Arms', init: '34 &amp; 55' },
      { key: 'seeds', label: 'Seeds', init: '0' },
    ],
    controls: [
      '<label class="game-range"><span>Turn</span><input type="range" id="game-turn" min="136000" max="139000" value="137508" /></label>',
      '<button class="game-btn game-btn-icon" type="button" id="game-golden" title="Back to the golden angle, 137.508°" aria-label="Back to the golden angle">&phi;</button>',
      '<label class="game-range"><span>Seeds</span><input type="range" id="game-seeds" min="300" max="3200" value="1800" /></label>',
      '<label class="game-range"><span>Size</span><input type="range" id="game-dot" min="10" max="60" value="26" /></label>',
      '<label class="sr-only" for="game-palette">Colour</label>',
      '<select class="game-select" autocomplete="off" id="game-palette"><option value="spectrum" selected>Spectrum</option><option value="ember">Ember</option><option value="ice">Ice</option><option value="mono">Mono</option></select>',
      '<button class="game-btn game-btn-icon" type="button" id="game-drift" aria-pressed="false" title="Let the turn walk on its own" aria-label="Drift the turn">&infin;</button>',
      '<button class="game-btn game-btn-icon" type="button" id="game-save" title="Save the figure as a PNG" aria-label="Save the figure as a PNG">&#10515;</button>',
    ],
    keys: [
    ...
    info: [
      {
        h: 'Why the radius goes as the square root',
        p: 'A disc of radius <em>r</em> has area &pi;<em>r</em>&sup2;, so holding the density even as seeds are added means the radius has to grow as &radic;<em>n</em>. That one choice is what keeps the figure looking the same from the middle to the rim rather than crowded in the centre. It also decides what the seed count does: the constant is derived from the target, so the last seed always lands on the rim and the slider changes how tightly the head is packed rather than how big it is.',
      },
      {
    ...
    faq: [
      ...
      },
      {
        ...
        a: 'Only so it is pleasant to leave running. The rotation is a single transform applied when the finished figure is copied onto the screen, so it costs the same whether there are 300 seeds or 3,200, and it changes nothing about the arrangement. If your system asks for reduced motion the rotation is off and drift starts held, and the button says so rather than sitting there labelled on while nothing moves.',
      },
      {
